...
const ExitModal = () => {
  ...
  const { visible } = useSelector((state) => state?.exitgame);
  ...
  const onMouseMove = useCallback(
    (e) => {
      ...
      dispatch(setExitVisible(newHeight > 90));
      setHeight(
  ...
  return (
    <Container $visible={visible}>
      ...
      <Body style={{ height: sizeCalculator(height) }}>
        <h1 className="roboto">Выйти из игры</h1>
        <p className="roboto">Вы точно хотите покинуть игру?</p>
        <div
          className="row"
          onClick={() => {
