...
import { setGiveUpModal } from "../redux/profileSlice";
...
const GiveUpModal = () => {
  ...
  const { giveUpModal: visible } = useSelector((state) => state?.profile);
  ...
  const onMouseMove = useCallback(
    (e) => {
      ...
      dispatch(setGiveUpModal(newHeight > 70));
      setHeight(
  ...
  const handleGiveUp = () => {
    if (!game?.id) return;
    const toastId = toast.loading("Идет отправка...");
    request
      .post(
        "api/game/player/surrender",
        { game_id: game?.id },
        {
          headers: {
  ...
  return (
    <Container $visible={visible}>
      ...
      <Body style={{ height: sizeCalculator(height) }}>
        <h1 className="roboto">Сдаться</h1>
        <p className="roboto">Вы точно хотите сдаться?</p>
        <div
          className="row"
          onClick={() => {
            dispatch(setGiveUpModal(false));
        ...
        >
          ...
          <button aria-label="Confirm Exit" onClick={handleGiveUp}>
            Да
          </button>
        </div>
      </Body>
